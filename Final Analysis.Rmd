...
```{r,  echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', results='hide'}
reg_3 = feols(new_solar ~ GDP + precip + Renewables + state_category + avg_temp, data = final_dataset)
summary(reg_3)
```
...
In this equation, I add a continuous variables for CO2 emissions per capita, and fully interact binary variables that take 1 if a state voted for Trump in the 2020 election and if a state has a Republican Governor. In this model, we see that a state that both voted for Trump in 2020 and has a republican governor is predicted to use 5.874% less renewable energy in comparison to states with just a trump vote or just a republican governor, or neither. However, this result is not statistically significant. We also observe that a 1 unit increase in CO2 emissions per capita is associated with a .0004 percent decline in renewable energy, significant at the 10% level. 
...
For my final regression, I use the amount of solar energy per capita as my dependent variable in an attempt to see which variables in my data set might be best at predicting the amount of solar energy a state uses. A 1 unit increase in the amount of precipitation a state recieves is associated with a -0.0040 unit decrease in the amount of solar energy per capita, a result that is to be expected. It is worth noting that blue states are the only political category whose effect is associated with a positive impact on the amount of solar energy. The coefficients for both red and purple states ar negative, although the standard error on the purple state's coefficient is large enough that we cannot say with confidence that it is on one side of zero or the other. A 1 unit increase in average temperature is associated with a 0.007 unit increase in the amount of solar energy per capita that a state uses. This result is suprisingly not statistically significantm and neither are any of the other chosen variables in this regression.
